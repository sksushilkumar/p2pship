...
#endif
	if (processor_config_get_int(config, P2PSHIP_CONF_AC_MAX_PATH, &max_path))
		return -1;
	
	freez(http_ac);
	if (http_ac = strdup(processor_config_string(config, P2PSHIP_CONF_AC_HTTP)))
		return 0;
	return -1;
}
...
ac_init(processor_config_t *config)
{
	...
	ASSERT_ZERO(ac_cb_config_update(config, NULL, NULL), err);
